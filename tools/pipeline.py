...
from config import STEP_REGISTRY
...
def render_pipeline_sidebar(col_names: list) -> tuple:
    ...
    if "Topic clustering" in selected_labels:
        ...
        col1, col2 = st.sidebar.columns(2, gap="small")
        cluster_n = col1.number_input("Clusters", min_value=2, max_value=20, value=5, key="cluster_n")
        cluster_n_init = col2.number_input("K-Means inits", min_value=1, max_value=20, value=10, key="cluster_n_init")
        col3, col4 = st.sidebar.columns(2, gap="small")
        cluster_max_df = col3.number_input("Max DF", min_value=0.1, max_value=1.0, value=0.95, step=0.05, key="cluster_max_df")
        cluster_min_df = col4.number_input("Min DF", min_value=1, max_value=10, value=1, key="cluster_min_df")
    ...
    return selected_labels, dup_columns, country_column, convert_code, translate_cols_list, target_lang, source_lang, cluster_params
...
def tool_data_pipeline():
    ...
    selected_steps, dup_columns, country_column, convert_code, translate_cols_list, target_lang, source_lang, cluster_params = render_pipeline_sidebar(col_names)
